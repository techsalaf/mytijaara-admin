                                            ...
                                            <tr>
                                                <td style="width:140px; vertical-align:middle; text-align:center">
                                                    <img class="mail-img-2"
                                                        src="{{ $order->store?->logo_full_url ?? \App\CentralLogics\Helpers::get_full_url('business', $logo?->value ?? '', $logo?->storage[0]?->value ?? 'public', 'favicon') }}"
                                                        alt="{{ $order->store?->name ?? '' }}">
                                                </td>
                                                <td style="vertical-align:middle; padding-left:16px">
                                                    @if ($order->store?->name)
                                                        <h3 class="mt-0 mb-1" style="font-size:18px">{{ $order->store->name }}</h3>
                                                    @endif
                                                    @if ($order->store?->address)
                                                        <div class="d-block" style="margin-bottom:2px">{{ $order->store->address }}</div>
                                                    @endif
                                                    @if ($order->store?->phone)
                                                        <div class="d-block" style="margin-bottom:2px">{{ $order->store->phone }}</div>
                                                    @endif
                                                    @if ($order->store?->email)
                                                        <div class="d-block">{{ $order->store->email }}</div>
                                                    @endif
                                                </td>
                                            </tr>
                                        </tbody>
