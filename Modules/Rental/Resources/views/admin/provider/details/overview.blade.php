...
@section('content')
    <div class="content container-fluid">
        ...
        @if($store->vendor->status)
            @php
                $reviewsInfo = $store->vehicle_reviews()->where('status', 1)->selectRaw('AVG(rating) as average_rating, COUNT(id) as total_reviews')->first();
                $average_rating = round((float) ($reviewsInfo->average_rating ?? 0), 1);
                $review_count = (int) ($reviewsInfo->total_reviews ?? 0);
                $order_stats = $store->trips()->selectRaw("
                    COUNT(*) as total_orders,
                    SUM(CASE WHEN trip_status = 'completed' THEN 1 ELSE 0 END) as delivered_orders,
                    SUM(CASE WHEN trip_status = 'canceled' THEN 1 ELSE 0 END) as canceled_orders
                ")->first();
        ...
        <div class="taxi-banner radius-10 mt-4 mb-20"
            ...
            <div class="taxi-info-wrapper d-flex flex-wrap flex-sm-nowrap gap-30px">
                ...
                <div class="taxi-info">
                    <h3 class="fs-20 fw-bold text--title mb-20 d-flex align-items-center gap-2">
                        {{ $store->name }}
    ...
    <div class="card mt-4 mb-4 p-4">
                        ...
                        <a class="order--card h-100" href="#">
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <span class="card-title text--warning">
                                    {{ number_format($canceled_orders > 0 ? ($canceled_orders / $total_orders) * 100 : 0) }}%
                                </span>
                            </div>
                        </a>
                        <!-- End Card -->
                ...
                <div class="info-notes-bg px-3 py-2 rounded fz-11  gap-2 align-items-center d-flex mt-20">
                    ...
                    <span>
                        {{translate('messages.Total_reviews')}}: {{ $review_count }}
                        <span class="mx-1">|</span>
                        {{translate('messages.average_ratings')}}: {{ $average_rating }}/5
                        <span class="mx-1">|</span>
                        @if (!is_null($performance_rate))
                            {{translate('This provider’s performance is rated as')}}
                        ...
                        @else
                            {{ translate('This provider does not have much available data.') }}
                        @endif
                    </span>
                </div>
            </div>
                            ...
                            <div class="card-body">
                                <div class="resturant--info-address">
                                    <div class="pdf-single" data-pdf-url="{{ $store->tin_certificate_image_full_url ?? asset('public/assets/admin/img/upload-cloud.png') }}">
                                         <div class="pdf-frame">
                                                        @php
                                                            $imgPath =  $store->tin_certificate_image_full_url ?? asset('public/assets/admin/img/upload-cloud.png');
                                                        @endphp
                                                        @if(Str::endsWith($imgPath, ['.pdf', '.doc', '.docx']))
                                                            @php
                                                                $imgPath =  asset('public/assets/admin/img/document.svg');
                                                            @endphp
                                                        @endif
