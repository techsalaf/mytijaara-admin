            ...
            @foreach($transactions as $t)
                <tr>
                    ...
                    <td class="text-center">
                        <div class="d-flex flex-column align-items-center justify-content-center">
                            ...
                            @if(!empty($t['trip_id']))
                                <a href="{{ route('admin.transactions.rental.trip.details', $t['trip_id']) }}" class="fs-12">
                                    {{ $t['earning_from'] }}
                                </a>
                            @endif
                ...
                @if(!empty($t['breakdown']))
                    <tr class="collapsing-tr d-none bg-light2">
                        <td></td>
                        <td colspan="4" class="pr-0">
                            <div class="mb-2">{{ translate('Trip Commission') }}</div>
                            <div>{{ $additionalChargeLabel }}</div>
                        </td>
                        <td class="text-right pl-0">
                            <div class="mb-2">{{ \App\CentralLogics\Helpers::format_currency($t['breakdown']['trip_commission'] ?? 0) }}</div>
                            <div>+ {{ \App\CentralLogics\Helpers::format_currency($t['breakdown']['additional_charge'] ?? 0) }}</div>
                        </td>
                    </tr>
                @endif
            @endforeach
