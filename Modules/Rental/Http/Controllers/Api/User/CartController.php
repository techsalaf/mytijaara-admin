...
class CartController extends Controller
{
    ...
    private function updateCartPrice($request, $user_id, $is_guest, $user_data = null)
    {
        ...
        $carts =  $this->cart->where('user_id', $user_id)->where('is_guest', $is_guest)->where('module_id', getModuleId($request->header('moduleId')))
            ...
            }, 'provider:id,name,address,tax,pickup_zone_id', 'provider.storeConfig:id,store_id,verified_seller', 'provider.discount' => function ($query) {
        ...
        $carts->each(function ($cart) {
            ...
            $cart->provider['verified_seller'] = Helpers::get_verified_seller_status($cart->provider, $cart->provider?->storeConfig);
            unset($cart->provider['storeConfig']);
